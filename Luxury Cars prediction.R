...
dataset = read.csv("C:/Users/Vishal/Documents/R/Luxury_Cars_Tutorial 6.csv")
View(dataset)
...
#Changing column names bcz its created symbolic problems in dataset. 
names(dataset)[9] <- "mpg"
names(dataset)[6] <- "EngineSize"
names(dataset)[10] <- "Weight"
names(dataset)[11] <- "Wheelbase"
names(dataset)[12] <- "Length"
View(dataset)
str(dataset)
...
library(Metrics)
mape(testData$Pred_MPG,testData$mpg)
#Regression Eqn:
#Mileage = 43.25 -2*(Type2) -1.26*(Origin1) -1.67*(Origin2) +2.22*(DriveTrain1) +
#0.08*(Wheelbase) -2.3*(Cylinders2) -3.06*(Cylinders1) -0.03*(Horsepower) -0.004*(Weight)
